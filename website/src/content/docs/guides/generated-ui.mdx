...
The template compiler reconstructs markup through explicit element and attribute allowlists before the DOM runtime assigns `innerHTML`. The SVG profile permits static geometry and text (`svg`, `g`, `path`, `rect`, `circle`, `ellipse`, `line`, `polyline`, `polygon`, `text`, `tspan`, `title`, and `desc`). Text and attribute interpolations are escaped.
