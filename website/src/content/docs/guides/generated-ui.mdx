...
The DOMPurify renderer is the recommended browser renderer. It applies Gruntend's explicit markup policy, sanitizes into a `DocumentFragment`, and commits it with `replaceChildren()`. For controlled integrations that need the compiled-HTML sink, use `createDomGeneratedUiRenderer()` from `gruntend-sdk/renderer/dom`. The older `mountGeneratedUi()` helper remains available from `gruntend-sdk/ui/dom` for compatibility.
