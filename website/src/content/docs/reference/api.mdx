...
`createDomPurifyGeneratedUiRenderer()` is exported from `gruntend-sdk/renderer/dom-purify` and is the recommended browser renderer. `createDomGeneratedUiRenderer()` is exported from `gruntend-sdk/renderer/dom` for controlled integrations.
