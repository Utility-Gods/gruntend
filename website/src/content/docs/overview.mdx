...
<Aside type="caution" title="Public beta — not production-ready">
  ...
  untrusted plans. Treat generated plans as untrusted input. Executor isolation does not replace
  least-privilege tools, runtime schema validation, authorization, and mutation
  checks in application handlers. Gruntend's execution boundaries are actively
  hardened as the project evolves; review release notes and evaluate each
  executor against your threat model before exposing sensitive capabilities.
</Aside>
