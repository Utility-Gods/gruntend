...
This guide uses Valibot, but Gruntend is not tied to one schema library. You can use Zod or any other validator that implements [Standard Schema](https://standardschema.dev/) for tool inputs and outputs.
...
<Aside type="note" title="Validation invariant">
  Invalid input never reaches the handler. Invalid successful output never
  returns to the plan. Expected failures use `err(...)`; throws remain the
  fallback for unexpected faults.
</Aside>
