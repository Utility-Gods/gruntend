...
export function listUsers(): User[] {
  return db
    .query<UserRow, []>("SELECT * FROM users ORDER BY user_id")
    .all()
    .map(rowToUser);
}
...
export function listMenus(): Menu[] {
  return db
    .query<MenuRow, []>("SELECT * FROM menus ORDER BY menu_id")
    .all()
    .map(rowToMenu);
}
...
export function getMenu(menuId: string): Menu | undefined {
  const menu = db
    .query<MenuRow, [string]>("SELECT * FROM menus WHERE menu_id = ?")
    .get(menuId);
...
export function createMenu(input: {
  ...
  const existing = db
    .query<
      MenuRow,
      [string]
    >("SELECT * FROM menus WHERE name = ? COLLATE NOCASE")
    .get(input.name);
...
export function listMenuItems(menuId: string): MenuItem[] {
  return db
    .query<
      MenuItemRow,
      [string]
    >("SELECT * FROM menu_items WHERE menu_id = ? ORDER BY item_id")
    .all(menuId)
    .map(rowToMenuItem);
}
...
export function getMenuItem(input: {
  ...
  const item = db
    .query<
      MenuItemRow,
      [string, string]
    >("SELECT * FROM menu_items WHERE menu_id = ? AND item_id = ?")
...
export function createMenuItem(input: {
  ...
  const existing = db
    .query<
      MenuItemRow,
      [string, string]
    >("SELECT * FROM menu_items WHERE menu_id = ? AND name = ? COLLATE NOCASE")
...
export function duplicateMenuItem(input: {
  readonly menuId: string;
  readonly itemId: string;
}): MenuItem {
...
export function deleteMenuItem(input: {
  readonly menuId: string;
  readonly itemId: string;
}): MenuItem {
...
function seedDatabase() {
  ...
  db.transaction(() => {
    for (const user of users)
      insertUser.run(user.userId, user.name, user.role, user.createdAt);
...
function nextId(table: string, column: string, prefix: string): string {
  const rows = db
    .query<{ readonly id: string }, []>(`SELECT ${column} as id FROM ${table}`)
    .all();
  let max = 0;
